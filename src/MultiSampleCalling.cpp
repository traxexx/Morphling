	...
	for( vector<string>::iterator mt = mei_type.begin(); mt != mei_type.end(); mt++ ) {
		for( vector<string>::iterator current_chr = chrs.begin(); current_chr != chrs.end(); current_chr++ ) {
			...
			if ( ExistDoneFile( sl_dir, site_list_base_name.c_str() ) ) {
				cerr << "Warning: [MultiSampleCalling()] exist done file: " << site_list_base_name << ", this chr & mei-type!" << endl;
				continue;
			}
...
// re-genotype each vcf
	for( vector<string>::iterator mt = mei_type.begin(); mt != mei_type.end(); mt++ ) {
		...
		for( vector< vector<string> >::iterator item_ptr = SampleList.begin(); item_ptr != SampleList.end(); item_ptr++ ) {
			...
			string sm = sample_name + "." + *mt;
			if ( ExistDoneFile( rg_dir, sm.c_str() ) ) {
				cerr << "  Warning: [MultiSampleCalling()] sample " << sample_name << ", meitype = " << *mt << " has been re-genotyped. Skipped!" << endl;
				continue;
			}
			string bam_name = (*item_ptr)[1];
			string vcf_prefix = (*item_ptr)[2] + "split/Hits";
			string refs_prefix = (*item_ptr)[2] + "QC/sref." + *mt;

			cout << "  Working on sample " << sample_name << " ..." << endl;
		// set numeric threshold according to qinfo generated by Morphling-discover
			string qinfo_name = (*item_ptr)[2] + "QC/QC.info";
			...
			for( vector<string>::iterator current_chr = chrs.begin(); current_chr != chrs.end(); current_chr++ ) {
				...
				if ( !section_status ) {
					cerr << "ERROR: [MultiSampleCalling()] Unable to set read section at: " << *current_chr << ", in file: " << bam_name << endl;
					exit(1);
				}
				...
				ReGenotypeSingleVcf(  SimplifiedSiteList[ mei_index ][*current_chr], single_REF_SEQ, rstats, samIn, samHeader, in_vcf_name, out_vcf_name);
			}
			samIn.Close();
...
// generate consensus vcf per chr: read each sample all mei-type then print the whole chr out
	cout << "Generating consensus vcf by merging all re-genotyped vcfs..." << endl;
	for( vector<string>::iterator current_chr = chrs.begin(); current_chr != chrs.end(); current_chr++ ) {
